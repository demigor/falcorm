...
public class DbSession(IServiceProvider sp, DbConnection conn, DbDialect? dialect = null)
{
  ...
  public readonly DbDialect Dialect = dialect ?? DetectDialect(conn) ?? throw new InvalidOperationException("Dialect not detected");
  ...
  static DbDialect? DetectDialect(DbConnection conn)
  {
    ...
    if (n.Length == 16 && n[0] == 'N') return DbDialect.Postgres;
    if (n == "SqlConnection") return DbDialect.Mssql;
    return null;
  }
